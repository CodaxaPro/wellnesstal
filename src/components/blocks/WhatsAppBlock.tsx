...
export default function WhatsAppBlock({ block }: BlockProps) {
  const [homepageWhatsAppData, setHomepageWhatsAppData] = useState<WhatsAppContent>(defaultContent)
  const [isLoadingHomepage, setIsLoadingHomepage] = useState(true)

  // Always fetch homepage WhatsApp block settings (like Footer does)
  useEffect(() => {
    ...
    fetch('/api/pages?slug=home')
      ...
      .then(data => {
        if (data.success && data.data?.blocks) {
          // Find WhatsApp block in homepage blocks
          const whatsappBlock = data.data.blocks.find((b: any) => b.block_type === 'whatsapp')
          if (whatsappBlock?.content) {
            setHomepageWhatsAppData(whatsappBlock.content as WhatsAppContent)
          } else {
            setHomepageWhatsAppData(defaultContent)
          }
        } else {
          setHomepageWhatsAppData(defaultContent)
        }
      ...
      .catch(() => {
        ...
        fetch('/api/pages?slug=index')
          ...
          .finally(() => setIsLoadingHomepage(false))
      })
      .finally(() => setIsLoadingHomepage(false))
  }, [])

  // Always use homepage WhatsApp settings (like Footer does)
  const content: WhatsAppContent = React.useMemo(() => {
    // Always use homepage WhatsApp block content
    const merged = {
      basic: { ...defaultContent.basic, ...homepageWhatsAppData.basic },
      appearance: { ...defaultContent.appearance, ...homepageWhatsAppData.appearance },
      message: { ...defaultContent.message, ...homepageWhatsAppData.message },
      display: { ...defaultContent.display, ...homepageWhatsAppData.display },
      availability: { ...defaultContent.availability, ...homepageWhatsAppData.availability },
      ctaBubble: { ...defaultContent.ctaBubble, ...homepageWhatsAppData.ctaBubble }
    }
    
    // Debug log to see what content is being used
    if (process.env.NODE_ENV === 'development') {
      console.log('[WhatsAppBlock] Rendering with content:', {
        buttonColor: merged.appearance.buttonColor,
        iconColor: merged.appearance.iconColor,
        blockId: block.id,
        homepageLoaded: !isLoadingHomepage
      })
    }
    return merged
  }, [block.id, homepageWhatsAppData, isLoadingHomepage]) // Include all dependencies
...
return
  ...
  // Show loading state when fetching homepage WhatsApp
...
return null
  ...
  // Original style render - but use custom colors if set
    ...
    return (
      <div className={`fixed bottom-6 ${positionClasses} z-50 flex flex-col items-center`}>
        ...
        <button
          ...
          className="w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg transition-all duration-300 relative group"
          style={{
        ...
        >
          <svg className="w-7 h-7" fill={content.appearance.iconColor} viewBox="0 0 24 24">
